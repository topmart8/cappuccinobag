  ...
  return <main className="crm-content">
    ...
    <div className="crm-detail-grid">
      <div className="crm-stack">
        ...
        {Array.isArray(inquiry.uploaded_files) && inquiry.uploaded_files.length ? <section className="crm-panel"><div className="crm-panel-header"><h2>安全附件</h2></div>{inquiry.uploaded_files.map((file, index) => <div className="crm-activity" key={`${file.path}-${index}`}><span className="crm-activity-dot" /><div><p>{file.name || `附件 ${index + 1}`}</p><small>{file.type || "file"} · {Number(file.size || 0).toLocaleString()} bytes · <a href={`/api/crm/inquiries/${inquiry.id}/attachments?file=${index}`} target="_blank" rel="noreferrer">打开 5 分钟签名链接</a></small></div></div>)}</section> : null}
