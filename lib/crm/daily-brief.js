...
export function generateDailySalesBrief({ customers = [], inquiries = [], activities = [], tasks = [], messages = [], reference_time } = {}) {
  ...
  for (const customer of customers.filter((row) => row.is_demo !== true)) {
    ...
    const actionable = Boolean(
      ...
      && (taskInfo.overdue.length || qualificationInfo.conflicts.length || qualificationInfo.nextQuestion
  ...
  const todayTopActions = priorityCustomers.slice(0, 5).map((item) => Object.freeze({
    rank: item.rank,
    customer_id: item.customer_id,
