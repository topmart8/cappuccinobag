...
export async function saveWebsiteInquiry(site, raw, uploadedFiles = []) {
  ...
  ].filter(([, value]) => clean(value, 400)).map(([label, value]) => `${label}: ${clean(value, 400)}`).join("\n");
  const input = {
    site,
    brand: brand.brand,
    ...
    product_category: clean(raw.product_category || raw.product_needed || raw.product || raw.productType, 180) || "Other",
