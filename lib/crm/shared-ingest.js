...
export async function deliverInquiryEmails(input, inquiry, draft, send = sendEmail) {
  ...
  const details = [
    ...
    ["Customization", input.customization], ["Message", input.message],
...
export async function ingestSharedInquiry(
  { siteSource, raw = {}, uploadedFiles = [] },
  dependencies = {},
) {
  const input = mapSharedInquiryPayload(siteSource, raw, uploadedFiles);
  const findExisting = dependencies.findExisting || findInquiryBySubmissionId;
  const existing = await findExisting(input.submission_id);
  if (existing) {
    return {
      customer: null,
      inquiry: existing,
  ...
  const deliver = dependencies.deliverEmails || deliverInquiryEmails;
  ...
  try {
    emailStatus = validateEmailStatus(await deliver(input, stored.inquiry, draft));
  } catch {
    emailStatus = "failed";
  }
  await setEmailStatus(stored.inquiry.id, emailStatus).catch(() => {});
