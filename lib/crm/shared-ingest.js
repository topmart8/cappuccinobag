...
import { resolveCustomerOutboundPolicy } from "./outbound-safety.js";
import {
...
export async function deliverInquiryEmails(input, inquiry, draft, send = sendEmail, raw = {}) {
  // The draft remains available for human review; this function only delivers the internal notification.
  void draft;
...
async function persistWorkflowRecords(input, stored, draft) {
  ...
  const configuredMode = process.env[
    input.site === "cappuccinobag" ? "CAP_INQUIRY_REPLY_MODE" : "NOV_INQUIRY_REPLY_MODE"
  ];
  const outboundPolicy = resolveCustomerOutboundPolicy(configuredMode);
  await supabaseRequest("ai_reply_logs", {
    ...
    body: {
      ...
      site: input.site,
      brand: brand.brand,
      mode: outboundPolicy.effective_mode,
      model: draft.model,
