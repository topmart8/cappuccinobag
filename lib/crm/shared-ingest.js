...
const OUTREACH_EXCLUSIONS = new Set([
  "blocked", "duplicate_review", "existing_customer", "old_customer", "supplier_non_buyer",
]);
...
export function attributionFrom(input = {}) {
  return {
    first_landing_page: clean(input.first_landing_page || input.firstLandingPage || input.landing, 1200) || null,
    ...
    first_visit_time: validTimestamp(input.first_visit_time || input.firstVisitTime || input.first_visit_at),
...
export function mapSharedInquiryPayload(siteSource, raw = {}, uploadedFiles = []) {
  ...
  return {
    ...
    product_category: clean(
      raw.product_category || raw.category || raw.product_needed || raw.product || raw.productType,
      180,
    ) || "Other",
    ...
    company_website: clean(raw.company_website || raw.website, 500) || null,
...
export async function deliverInquiryEmails(input, inquiry, draft, send = sendEmail, raw = {}) {
  ...
  const customerReplyAllowed = !OUTREACH_EXCLUSIONS.has(input.identity_status);
  if (enabled && safeAuto && customerReplyAllowed && !draft.human_review_required) {
    await send({
      to: input.email,
  ...
  const workflowWrites = [
    supabaseRequest("activities", {
      ...
      body: {
        ...
        metadata: {
          ...
          identity_status: stored.identityStatus,
          identity_match_method: stored.matchMethod,
        },
      },
    }),
    supabaseRequest("tasks", {
      ...
      body: {
        ...
        source: "website",
        owner: stored.inquiry.owner || stored.inquiry.assigned_owner || "unassigned",
        title: stored.identityStatus === "blocked"
          ? "Review suppressed inbound inquiry"
          : `Review ${brand.brand} inquiry ${stored.inquiry.inquiry_number}`,
        description: draft.recommended_action,
        priority: stored.identityStatus === "blocked" ? "urgent" : "normal",
        status: "open",
      },
    }),
  ];
  if (!OUTREACH_EXCLUSIONS.has(stored.identityStatus)) {
    workflowWrites.push(supabaseRequest("email_drafts", {
      method: "POST",
      body: {
        customer_id: stored.customer.id,
        inquiry_id: stored.inquiry.id,
...
export async function ingestSharedInquiry(
  ...
  const deliveryEnabled = dependencies.deliveryEnabled
    ?? process.env.CRM_AUTOMATED_EMAIL_ENABLED === "true";
  let emailStatus = "skipped";
  if (deliveryEnabled) {
    try {
      emailStatus = validateEmailStatus(await (deliver
        ? deliver({ ...input, identity_status: stored.identityStatus }, stored.inquiry, draft, raw)
        : deliverInquiryEmails(
          { ...input, identity_status: stored.identityStatus },
          stored.inquiry,
          draft,
          sendEmail,
          raw,
